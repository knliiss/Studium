...
public interface SubjectTeacherRepository extends JpaRepository<SubjectTeacher, UUID> {
    ...
    boolean existsByTeacherIdAndSubjectIdIn(UUID teacherId, Collection<UUID> subjectIds);

    List<SubjectTeacher> findAllByTeacherIdOrderByCreatedAtAsc(UUID teacherId);
